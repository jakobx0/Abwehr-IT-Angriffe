...
> 
> - 4.2 Sie können Ergebnisse gezielt für eine bestimmte Eingabedatei anzeigen lassen:
> ```bash
> john macos.neu --show
> john /media/sf_passwords/data/windows/plain --format=NT --show
> ```
> Achten Sie dabei besonders auf die ersten beiden Spalten, in denen Nutzername und Passwort ausgegeben werden.
> 
> 4.3 Wenn Sie durch z. B. Profiling genügend Informationen zu einer Person gesammelt haben, können Sie gezielt Passwörter vermuten. Testen Sie dies mit dem einfachen Profiler CUPP (Common User Passwords Profiler).
> 
> - 4.3.1 Damit die generierten Daten nicht unnötig auf Ihrer regulären Festplatte landen, kopieren Sie das Tool in das flüchtige Verzeichnis `/dev/shm10` und führen es dort aus:
> `cp -r /media/sf_passwords/tools/cupp/ /dev/shm/`
> `cd /dev/shm/cupp`
> `python3 cupp.py -i`
> 
> ![[Pasted image 20251102114801.png]]
> 
> - 4.3.2 Beantworten Sie die Fragen im interaktiven Modus mit echten oder fiktiven Werten. Falls eine Information nicht vorliegt, drücken Sie einfach Enter , um das Feld zu überspringen.
> - 4.3.3 Im Anschluss können Sie das erzeugte Wörterbuch anschauen und die Anzahl der generierten Einträge prüfen:
>```bash
>cat name.txt
>wc -l *.txt (Zeigt die Anzahl generierter Passwortkandidaten pro Datei an)
>```
>4.3.4 Löschen Sie die erzeugten Dateien danach sicher, damit keine sensiblen Wortlisten auf demSystem verbleiben:
>` shred -zu *.txt (Sicheres Löschen)`

>[!example] 5 Sicherheit von Schlüsseln in Funkverbindungen (WPA2-PSK)
>Es sollte bekannt sein, dass veraltete Funkverschlüsselungen (WEP) nicht mehr verwendet werden sollten, da die Algorithmen selbst grundlegende Schwächen aufweisen. Moderne Verfahren wie WPA2 sind allgemein zu bevorzugen, auch wenn fehlerhafte Implementierungen oder Fehlkonfigurationen Sicherheitsprobleme erzeugen können. WPA3 behebt viele der bekannten Probleme, ist aber noch nicht überall verfügbar. In der Übergangszeit sind daher weiterhin viele WLANs mit WPA2 im Einsatz. Bei WPA wird oft vom Pre-Shared Key oder kurz PSK gesprochen. Diese lesbare Passphrase aus 8 bis 63 ASCII-Zeichen, versteht man umgangssprachlich als Passwort des Netzwerkes. In der Praxis wird bei Heimnetzwerken aus dem PSK mittels einer Schlüsselableitungsfunktion ein so genannter Pairwise Master Key (PMK) abgeleitet, der das eigentliche Geheimnis des Netzwerkes als Hauptschlüssel darstellt. Die SSID dient dabei als Salt bei der Erzeugung. Für die eigentliche Verschlüsselung des Datenverkehrs
werden weitere Schlüssel (PTK: Pairwise Transient Key, GTK: Group emporal Key) abgeleitet.
>
>4 Way-Handshake:
>Nach der Authentifizierung und Assoziierung zwischen Client und Access Point findet mittels des 4-Way-Handshake ein gegenseitiger Nachweis und die Ableitung der Schlüssel für den verschlüsselten Datenverkehr statt. Dieser Ablauf erfolgt in Form von **EAPoL**-Frames (Extensible Authentication Protocol over LAN) und enthält Integritätsnachweise, die auf dem PMK bzw. aus dem PMK abgeleitete Schlüsseln beruhen. Fängt ein Angreifer den 4-Way-Handshake ab, so erhält er die Werte und Integritätsnachweise, die es ihm erlauben, offline zu prüfen, ob eine Kandidaten-Passphrase korrekt ist. Ein Offline-Angriff auf EAPOL-Frames ist damit im Wesentlichen mit einem Hash-Cracking-Angriff vergleichbar: Man testet viele Passphrasen, bis die berechnete „Beweismeldung“ (Integritätsnachweis) mit dem im Mitschnitt enthaltenen Wert übereinstimmt 
>
>Für diese Aufgabe steht Ihnen ein Netzwerkmitschnitt bereit, in dem ein 4-Way-Handshake stattgefunden hat. Beachten Sie, dass Mitschnitte einer dritten Partei unvollständige Pakete enthalten können. Hinweis: Das Abhören fremder Netze ohne Zustimmung ist unethisch und illegal.
>
>5.1 Öffnen Sie den Netzwerkmitschnitt in Wireshark und machen Sie sich mit den Daten vertraut: `cd /media/sf_passwords/data/wlan` wireshark filtered.pcap & (Mit dem „&“ blockiert Wireshark das Terminal nicht) EAPOL-Frames bzw. Key-Frames bilden den 4-Wege-Handshake. Prüfen Sie mit dem Anzeigefilter eapol, ob alle notwendigen Pakete aufgezeichnet wurden. Achten Sie besonders darauf, ob ein kompletter Handshake vorhanden ist. Unvollständige oder fehlende Frames verhindern einen verlässlichen Offline-Angriff.
>
>![[Pasted image 20251103165212.png]]
>
>5.2 Falls ein Schlüsselaustausch11 (Handshake) im Mitschnitt vorhanden ist, können Sie versuchen, die gemeinsam genutzte Passphrase (PSK) zu ermitteln. Wählen Sie dafür eine Kandidaten-Passphrase aus einem Wörterbuch (oder per Bruteforce) und prüfen Sie offline, ob sich daraus die im Mitschnitt enthaltenen Integritätsnachweise reproduzieren lassen. Prüfen Sie zunächst, ob und für welche SSID ein Handshake im Mitschnitt vorhanden ist:
>`aircrack-ng filtered.pcap`
>
>![[Pasted image 20251103165430.png]]
>
>5.3 Starten Sie einen Wörterbuchangriff mit einem von John the Ripper mitgelieferten Wörterbuch: `aircrack-ng filtered.pcap -w /usr/local/share/john/password.lst` Nach kurzer Zeit sollte eine Ausgabe wie folgt erscheinen, falls ein Treffer vorliegt: KEY FOUND! [ Passwort ] (Das Passwort (PSK) steht in den eckigen Klammern) Beachten Sie, dass die Erfolgsaussicht stark von der Qualität des Wörterbuchs und der Komplexität der Passphrase abhängt.
>
>![[Pasted image 20251103165838.png]]
>
>5.4 Da Sie nun den PSK für den aufgezeichneten Netzwerkmitschnitt ermittelt haben, können Sie Wireshark verwenden, um den aufgezeichneten Datenverkehr für Sie zu entschlüsseln. Dadurch werden die bislang kryptischen Data-Frames mit lesbarem Inhalt versehen.
>- 5.4.1 Entfernen Sie gegebenenfalls aktive Anzeigefilter in Wireshark, damit Sie alle Pakete sehen.
>- 5.4.2 Öffnen Sie das Einstellungsmenü [ Bearbeiten | Einstellungen... | Protocols | IEEE 802.11 ] und aktivieren Sie im unteren rechten Bereich die Entschlüsselung (Enable decryption), falls dies noch nicht gesetzt ist.
>- 5.4.3 Öffnen Sie den Dialog zum Bearbeiten der Schlüssel (Decryption Keys Editieren...) und fügen Sie über das Pluszeichen einen neuen Schlüssel hinzu.
>- 5.4.4 Wählen Sie als Schlüsseltyp „wpa-pwd“12 (WPA Passwort) und tragen Sie den Eintrag im Format `psk:ssid` Enter ein.
>
>![[Pasted image 20251103175045.png]]
>
>5.5 Nach Bestätigung der Dialoge stehen Ihnen nun die entschlüsselten Frames zur Verfügung. In diesem Mitschnitt können Sie beispielsweise ARP-Pakete (Pakete 120–122) und DNS-Anfragen (Pakete 124–125) sowie unverschlüsselten HTTP-Datenverkehr (Paket 195) analysieren. Je nach Qualität des Mitschnitts kann es jedoch vorkommen, dass nicht alle erforderlichen Fragmente für die Rekonstruktion eines Pakets erfasst wurden (z. B. Paket 213). In solchen Fällen ist eine vollständige Analyse dieses Pakets nicht möglich. 
>
>Hinweis: Beachten Sie, dass nach der 802.11-Entschlüsselung einzelne Anwendungsschichten weiterhin verschlüsselt sein können, zum Beispiel TLS. Die 802.11-Entschlüsselung macht nur die MAC-Layer-Payload sichtbar.
>
>5.6 Wenn man sich die Pakete ein wenig genauer anschaut und eine Unterscheidung zwischen noch verschlüsselten und bereits entschlüsselten Daten treffen möchte, kann ein geeigneter Anzeigefilter helfen, nur die entschlüsselten Daten anzuzeigen. Welchen der folgenden Filter würden Sie dafür am ehesten als sinnvoll erachten und warum oder warum nicht?
>```bash
>ip
>ip || arp
>llc
...
>```
>
>5.7 Das Paket 496 enthält ein erfolgreich übertragenes Bild (JPEG JFIF image) von einem Webserver an einen Client im Netzwerk. Sie können dieses Bild direkt über Wiresharks interne Vorschau anzeigen lassen:
>- 5.7.1 Markieren Sie das Paket 496 in der Paketliste.
>- 5.7.2 Öffnen Sie in der Detailansicht (Fenster mit den ausklappbaren Protokollfeldern) den letzten
>Protokolleintrag „JPEG File Interchange Format“.
>- 5.7.3 Öffnen Sie das Kontextmenü und wählen Sie [ Paketbytes anzeigen... ]. Alternativ können Sie die Tastenkombination Strg+Shift+O verwenden.
