...
>```
>
Erklärung: Damit ersetzen Sie die Standardregeln des Single-Modes (Phase 1) durch Ihre
eigene, einfache Regel. In der angegebenen Regel nutzen Sie das Wissen8, dass der Nutzer
häufig den Buchstabe e durch eine 3 ersetzt (se3) und außerdem haben Sie gesehen, dass
er am Ende seiner Passworteingabe immer ein Ausrufezeichen verwendet ($!).
3.5.3 Speichern Sie die Datei mit Strg + O Enter Strg + X .
Hinweis: Die lokale Konfiguration wird nur verwendet, wenn john im gleichen Verzeichnis
gestartet wird.
3.6 Starten Sie den Angriff mit Ihren vorbereiteten Einträgen und Regeln:
john macos.neu
3.7 Wie realistisch sind die Chancen, das Passwort ohne Hintergrundwissen per reinem Bruteforce
zu finden?
