...
>
>6. Die SSID (der Name) eines versteckten Netzwerks wird nur übertragen, wenn sich ein Client verbindet. Sie können passiv warten oder einen verbundenen Client aktiv zur erneuten Authentifizierung zwingen. Öffnen Sie ein neues Terminal und führen Sie, folgenden Befehl aus, um gezielte Deauthentication-Pakete zu senden: `sudo aireplay-ng --deauth 10 -a BSSID -c STATION wlanXmon` Der Befehl sendet Frames mit gefälschter MAC-Adresse vom AP an den Client, wodurch der Client zur Neuverbindung gezwungen wird. Beim Neuaufbau der Verbindung überträgt der Client die SSID, die Sie dadurch aufzeichnen können.
>
...
>
>1. Filtern nach Probe **Requests**: `wlan.fc.type_subtype == 0x04`
...
> 	- Privacy Leak: Aktive Suchanfragen können identifizierende Merkmale preisgeben. Besonders problematisch sind sogenannte Directed Probes, bei denen ein Client gezielt nach einer bestimmten SSID sucht. Aus solchen Probes lassen sich Rückschlüsse auf frühere Verbindungen, eingesetzte Dienste oder sogar einzelne Geräteprofile ziehen. Achten Sie darauf, ob Sie gezielte SSID-Abfragen oder wiederkehrende SSID-Muster finden. Beachten Sie außerdem, dass viele Geräte MAC-Randomisierung nutzen, was eine stabile Zuordnung erschweren kann.
> 
> 2. Filtern nach Probe **Responses**: `wlan.fc.type_subtype == 0x05`
...
> 
> 3. Deauthentication-Angriff: 
> 	 - alle Filter entfernen
> 	 - `I/O-Graph` -> `wlan.addr[0:3] == 3C:71:BF` -> zeigt nur Daten von Geräten mit dieser Herstellerkennung
...
> 
> 4. Ergänzen Sie einen zweiten Graphen mit dem Spaltennamen „Deauth“ und dem Filter `wlan.fc.type_subtype == 0x0c`. 
...
>
>5. Öffnen Sie über das Menü `[ Wireless | WLAN Traffic ]` 
