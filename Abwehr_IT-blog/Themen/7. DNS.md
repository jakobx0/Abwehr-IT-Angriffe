...
---
#### DNS Zone und DNS im Unternehmen
>[!abstract] DNS Zones
...
>
>![[Pasted image 20251123160940.png | 550]]
#### Angriffe DNS
>[!danger]
 Angriffe auf DNS haben den Zweck die Zuordnung zwischen einem Domainnamen und der zugehörigen IP-Adresse zu fälschen. Dies dient dazu, Datenverkehr unbemerkt zu einem anderen Computer zu lenken, zum Beispiel um einen Phishing- oder Pharming-Angriff durchzuführen. Wird der Datenverkehr auf ein nicht erreichbares Ziel gelenkt, kann hiermit ein Denial-of-Service-Angriff durchgeführt werden. Die folgenden Begriffe entspringen unterschiedlichen Angriffsmethoden, verfolgen jedoch denselben Zweck:
>
...
>
>>[!example] DNS Cache Spoofing
>> Durch Korruption der DNS-Cache-Daten eines DNS Servers, werden kompromittierte Informationen in das DNS eingeschleust (Juli 2008 Kaminsky Angriff).
>>- Angreifer fragt einen rekursiven Nameserver nach nicht existierenden Domain ab und initiiert damit eine DNS Abfrage beim Resolver.
>>- Der Resolver wird durch eine DoS Attacke stillgelegt. Und der Angreifer sendet eine gefälschte Antwort vom Resolver, die aus einem NS Resource Record und einem Glue Record besteht, der auf einen Host des Angreifers zeigt.
...
>>- Der rekursive Nameserver speichert die falsche IP-Adresse als Adresse für die gespoofte Domain.
>>- Logins, Passwörter, Kreditkartennummern des Benutzers können erfasst werden
>
>### Ettercap
>>[!info] Angriffe auf DNS
...
>
>>[!warning] DNS Spoofing mit Ettercap
>>Diese Art von Angriff führt dazu, dass der Nameserver eine falsche IP-Adresse zurückgibt und den Datenverkehr an einen anderen Computer umleitet. In Ettercap befinden sich einige nützliche Plug-Ins. Eines dieser Plug-Ins ist "dns_spoof". Folgende zwei Schritte sind nötig um den DNS Eintrag am Ziel zu spoofen.
>>1. Bearbeitung der Datei /usr/share/ettercap/etter.dns um die notwendigen Daten
>>2. einzutragen:
>>![[Pasted image 20251124112127.png | 450]]
>>3. Ausführung des Spoofing Kommandos mit dem Spoofing Plugin
>>![[Pasted image 20251124112248.png | 550]]
>>
>>Befehlsübersicht:
>>![[Pasted image 20251124112311.png]]
>
>
>##### DNS Tunneling
...
>
>`Erweiterung des ganzen -> DNS Amplification Attack`
>
>##### Refelective Amplification DNS Attack
>Bei der DNS Amplification Attack wird ausgenutzt, dass Nameserver in bestimmten Fällen auf kurze Anfragepakete mit sehr langen Paketen antworten. Eine 60 Bytes lange Anfrage kann in bestimmten Fällen eine mehr als 3.000 Bytes lange Antwort provozieren. Es liegt also ein Verstärkungsfaktor von mehr als 50 vor. Mittels IP-Spoofing wird diese Antwort auf die IP-Adresse des Opfers gelenkt. Werden jetzt zudem diese Anfrageintervalle intensiviert kann dies ein Opfer ausschalten.
>
>##### Fast flux DNS 
...
>[!success] Absicherung und Abwehr von Angriffen auf DNS 
