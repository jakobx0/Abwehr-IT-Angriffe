...
>>- `udp && frame.len > 1000` -> zeigt UDP Pakete mit Frame Länge > 1000 an 
>>- `http.request.uri` -> Zeigt alle HTTP-Pakete, die eine **URI (Uniform Resource Identifier)** enthalten
>>- `tcp.flags.ack == 1 && tcp.flags.syn == 1` -> offene tcp Ports anzeigen
>>- `tcp.flags.reset == 1 && tcp.flags.ack == 1` -> geschlossene ports anzeigen
>>- `tcp.flags.reset == 1` -> zeigt alle Pakete in welchem das RST Flag gesetzt ist
>>- `arp.duplicate-address-detected` -> ARP Poisoning
>>- `dns.qry.name contains "chemnitz"` -> zeigt Pakete bei denen im DNS teil der Query Name "chemnitz" vorkommt. 
>>- `dns.flags.response == 1` -> zeigt nur DNS Responsen an
>>- `tcp.analysis.retransmission` -> Erneut gesendete tcp Pakete
>>- `arp.opcode == 1` -> zeigt arp request an
>>- `arp.opcode == 2` -> zeigt arp reply an
>>- `tcp.len > 0` -> zeigt Pakete die Daten transportieren nicht nur ACK
>>- `http.host contains "hs-mittweida.de"` -> zeigt http-traffic bei dem der Host "hs-mittweida.de" ist  
>>- `vnc.auth_result` oder `vnc[0:4] = 00:00:00:00` -> zeigt VNC Auth Erfolg an
>>- `s7comm-plus.data.function == 0x0542` -> zeigt Funktion SetMultiVariables an 
>
>>[!tip] WLAN/801.11
>>WLAN/802.11 Display‑Filter (Wi‑Fi‑Praktikum)
>>
...
>> - SSID exakt: `wlan.ssid == "eduroam"`
>> - SSID enthält Teilwort: `wlan.ssid ~ "edu"`
>> - Info‑Spalte enthält Teilwort: `_ws.col.info ~ "edu"`
>
