#### Grundlagen
>[!info]
>- Datenübertragung von Web und Dienste über http in Layer7
>- zentrale Bestandteile des Internets
>- Webanwendungen nicht auf Gerät installiert
>- interagieren ausschließlich mit Server
>- benötigt Web-Anwendung Zugriff auf Funktionen des BS bspw. will im Client eine Hardwarekomponente ansprechen, wird die Browser Engine benötigt
#### Http-Protokoll
>[!abstract]
>- Übertragung von daten über ein Netzwerk
>- basiert auf TCP/IP -> Layer 7
>-Funktioniert nach Anfrage-Antwort-Prinzip
>- Ansprache von Anwendungsschicht durch Webanwendungen, wie z.B. Webbrowsern
>- Anfragen und Antworten sind im ASCII-Format angegeben
>
>![[Pasted image 20260108134538.png | 600]]
>
>>[!question] Kommunikation
>>1. **HTTP-Connect:**Client baut TCP/IP-Verbindung zu HTTP-Server über dessen TCP-Port 80 auf.
>>2. **HTTP-Request: **Client sendet Anfrage an Server und fordert über URL adressierte HTML-Datei an.
>>3. **HTTP-Response:** Server empfängt eingehenden HTTP- Request und überträgt adressierte HTML-Datei an Client oder antwortet mit Fehlermeldung.
>>4. **HTTP-Close:** Falls weder Client (HTTP-Request) noch Server (HTTP-Response) Aufrechterhaltung TCP/IP-Verbindung erbeten, z.B. um weitere Bilder der Webseite herunterzuladen, baut Server Verbindung zum Client ab.
>>
>>![[Pasted image 20260108135635.png | 500]]
>
>Nachrichtenstruktur von Anfragen und Antworten bei HTTP bestehen aus:
>- Header mit Metainformationen über die zu sendenden Daten
>- Payload mit dem eigentlichem Inhalt
>
>#### Abfragemethode
>- *GET*: Abfrage von Daten vom Server durch den Request
>- *POST*: Datenänderung/Übermittlung zum Zugangsdaten durch den Request
>>
>- HEAD: zum Anfordern eines Response-Headers
>- PUT: zur Erstellung/Änderung einer Datei auf dem Server
>- OPTIONS; zur Abfrage der erlaubten Methoden
>- DELETE: zum Löschen einer Datei auf dem Server
>- TRACE: zur Verfolgung von Anfragen, die über einen Proxyserver laufen
>- CONNECT: um Kommunikationstunnel mit einem Proxyserver aufzubauen 

---

<div style="page-break-before: always; min-height:1px;">&nbsp;</div>

---
### Bausteine von Webanwendungen
>[!abstract]
>- **Unterteilung in:**
>	- Client -> ruft auf
>	- Server -> stellt zu Verfügung
>- **Frondend**:
>	- Zugriff auf Anwendung bereit gestellten code (bsp. js)
>	- beliebig veränder und ergänzbar innerhalb des Clients
>- **Serverseitiges Backend:**
>	- zur Verwaltung und Administration der Anwendung und Bereitstellung einer Anbindung an Dateisysteme, DB und Dienste
>	- Serverseitiger Code (PHP) nur vom Server ausgeführt
>>
>- Drei Schichten-Architektur
>	- Benutzerschnittstelle/View layer
>		- HTML, CSS, javaScript ...
>	- Geschäftslogik / Business Logic Layer
>		- Verarbeitung von Client- und Serverdaten mit serverseitiger Programmiersprache wie bspw. PHP, Ruby, Perl, JAVA
>	- Datenhaltung/ Data Access Layer
>		- DBMS -> MySQL, PostgreSQL...
### Document Object Model
>[!abstract]
>- Dastellung in Baumstruktur
>
>![[Pasted image 20241211143111.png | 550]]
### Document Object Model und Same Origin Policy (SOP) 
> [!warning]
> - Verbot beim Besuch einer Website weitere Inhalte von anderen Servern nachzuladen.
> - Daten werden nur von demselben Server bezogen, ohne das Nachladen externer Inhalte
 >- sonst -> <frames.bank_frame.document.getElementById("Kontostand").value>
#### Top 10 Angriffe gemäß OWASP
>[!info]
>Generell zwei Angriffsvektoren um Webanwendungen anzugreifen:
>- Zugriff auf den Server
>- Server Ausführung eingeschleusten Code
>
>![[Pasted image 20260108145641.png | 450]]
>
>![[Pasted image 20260108145709.png | 450]]
## Zustandsbasierte Angriffe
>[!abstract] 
>Verfügbare Informationen von Webanwendungen
>- Übertragungs Protokoll HTTP -> zustandslos
>- Seite wird ohne Informationen, von wo der Benutzer kam und wohin er gehen kann
>- Nachverfolgung der Aktionen eines Benutzers und deren Verknüpfung (Zustandsinformationen) durch Webanwendung selbst
>- Zusammenfassung mehrerer Requests eines Benutzers als Session (Sitzung)
>- Identifizierung eins Benutzer mit einer Session-ID (Sitzungs-ID), zur Wiedererkennung nach mehreren Request
>
**Verwaltung der Zustandsinformationen durch Webanwendung**
>- Zustandsinformationen auf dem Server gespeichert und Identifikation Benutzer über Session-ID
>- Zustandsinformationen auf dem Client gespeichert
>
>Unabhängig vom Ansatz müssen Informationen auf Client gespeichert werden entweder Zustandsinformationen oder Session-ID
>
>**Methoden:**
>- versteckte Formularfelder -> `<input name="id" value="1234" type="hidden">`
>- Parameter in der URL -> http://www.webanwendung.example/editprofil.cgi?id=456
>- Cookies
>
>>[!danger]
>>Nutzung von Zustandsinformationen um Code einzuschleusen oder Sessions übernehmen:
>>
>>#### Cookie Poisoning
>>- Änderungen von Cookie Inhalten zur Manipulation von Session ID‘s oder auch das Verfallsdatum
>>- Bei fehlender Plausibilitätsprüfung werden geänderten Einträge genutzt
>>#### URL Jumping
>>- Angreifer verlässt vorgesehene Reihenfolge der Web-Seiten und nutzt Funktionen, die noch garnicht zugänglich sein sollten
>>- Beispiel: Ändern von Bestellinformationen (Bestellmengen) nach dem Bezahlvorgang
>>#### Session Hijacking
>>- Ausspähen Session ID eines Benutzers und danach als dieser Benutzer auszugeben (Cookie, URL ID, verstecktes Feld)
>>- Spezialfall des Session Hijacking ist „Authorization bypass“ 
>>#### Session Fixation
>>- Angreifer erstellt eigene Session mit eigener Session ID
>>- Session ID des Angreifers wird von einem anderen Benutzer authentifiziert
>>- Angreifer hat Zugriff auf Daten des Benutzers in eigener Session (Bsp: Bezahlvorgänge) und „arbeitet“ in seinem Namen
### SQL Injection
>[!abstract]
SQL-Injection : Einschleusen von eigenen Datenbankstatements in eine Webanwendung
>
>- Basierend auf dynamisch zusammengesetzten SQL-Anweisungen aus Benutzereingaben und vorgefertigten Statements
>- Überprüfung mittels JavaScript getätigte Eingaben vor Weitergabe an Datenbank
>- Oft Vergessen auch die Serverseite vor unerwünschten Abfragen zu schützen
>- Abfangen von Übertragungen an den Web Server und in verfälschter Form weitergeben
>
>Besonderheit bei Verarbeitung einer SQL-Anweisung: **SQL Interpreter** erhält Eingaben direkt und führt entsprechende Anweisung aus.
>Beispiel:
>```
>String query = "SELECT name FROM kunden WHERE kNr = $_GET['input']";
>```
>
>Sucht Benutzer nach Kunden mit Kundennummer 142, sieht erstellte SQL-Anweisung so aus:
>```
>String query = "SELECT name FROM kunden WHERE kNr = 142";
>```
>
>Webanwendung sendet Query in EINER Stringvariablen gespeichert zur Verarbeitung an den SQL Interpreter.
>
><div style="page-break-before: always;"></div>
>
>>[!danger] Angriffsmethoden
>>Setzen meistens innerhalb der WHERE-Klausel an:
>>- Platzierung immer wahrer Anweisungen, können Datenextrahieren
>>- Häufig in Kombination, in der hinderliche Befehle für die Ausführung der SQL Injektion auskommentiert werden.
>>
>> ![[Pasted image 20250201172314.png]]
>>>[!example] 
>>>#### Tautlogies
>>>- Tautologien sind immer wahrer Aussagen. Zusammensetzung so, dass ihre Bedingungen immer wahr werden, unabhängig von nicht erfüllten Teilbefehlen (auch SQL-Tautologien genannt).
>>>- Angriffsziele: Datenextraktion, Umgehung der Authentifizierung, Schwachstellenidentifizierung
>>>
>>>#### Illegal/Logical Incorrect Queries
>>>- Anhand zurückgegebener Fehlermeldungen lassen sich Informationen über Datenbank und Struktur abgreifen. Können potentiellen Angreifer helfen, verwundbare Stellen innerhalb Datenbanksystems zu ermitteln.
>>>- Angriffsziele: Datenbankschema und Schwachstellen identifizieren
>>>
>>>#### Union Queries
>>>- Verkettung Ergebnisse schadhafter und normaler SQL-Abfragen mit Hilfe UNION, um Daten anderer Tabellen zu erhalten. Beide Anweisungen benötigen exakt selbe Struktur Tatsächliche Tabellennamen müssen bekannt sein, sowie die Datentyp und Anzahl der Originalabfrage zurückgegebenen Spalten.
>>>- Angriffsziele: Datenextraktion, Umgehung der Authentifizierung
>>>
>>>#### Stored Procedures
>>>- Zusätzliche Abstraktionsschicht, die es Anwendern erlaubt, eigene, komplexe SQL-Anweisungen im Datenbanksystem vorzuhalten. IdR. aus mehreren Anweisungen bestehend, nach Speicherung, genauso abrufbar wie eingebaute Systemfunktionen. Nach Herausfinden des Datenbanktypen auch versuchen Stored Procedures zu ermitteln.
>>>- Angriffsziele: Remote-Code-Ausführung, Privilegien-Eskalation
>>>
>>>#### Alternative Encoding Obfuscation
>>>- Methode zur Erschwerung der Entdeckung einer Injektionsattacke durch Verwendung einer alternativen Kodierung (hexadezimal, ASCII, Unicode) durch Erkennungsregeln von Web Application Firewalls (WAF). Kombinierbar mit den verschiedensten Angriffsarten.
>>>- Angriffsziele: Verschleierung, WAF-Täuschung
>>>
>>>#### Piggy-backed Queries
>>>- Ursprüngliche, erste SQL-Anweisung nicht verändert, sondern hinter Query-Demilimiter „;“ durch weitere Anweisung ergänzt. Beide Anweisungen können auf Datenbank ausgeführt. Angriff nur möglich, wenn mehrere Abfragen innerhalb einer Anweisung erlaubt
>>>- Angriffsziele: Datenextraktion, Datenmanipulation, Remote-Code-Ausführung[
>
>>[!success] 
>>#### Schutzmaßnahmen
>>- **Escaping:** -> Steuerzeichen maskieren
>>- **Input Validation**: Validierung von Benutzer Eingaben
>>- **Prepared Stratments:** Parametrisierter Aufruf vorbereiteter SQL-Anweisungen
>>- **Stored Procedures**: Gespeicherte Prozeduren
>>
>>Ein guter Grundsatz ist, dass mindestens zwei und im besten Fall alle der hier genannten Ansätze implementiert werden sollten
>>
>>#### Escaping und Input Validation
>>**Escaping**: Maskieren wichtiger Steuerzeichen
>>- Steuerzeichen wie Anführungszeichen bergen besonders großes Angriffsrisiko um aus SQL Anweisungen auszubrechen.
>>- Backslash: nachfolgender Zeichen kein Steuerzeichen ist.
>>- Wildcard-Zeichen ( _oder %) sollten im Kontext von LIKE Befehlen maskiert werden
>>**Escape Funktionen:**
>>- mysql_real_escape_string()
>>- addslashes()
>>
>>**Inputvalidierung**: (Client & Server!) Überprüfung getätigter Eingaben auf Übereinstimmung mit vordefinierten Standards. Reicht vom Abgleich einzelner Zeichen, komplexe reguläre Ausdrücke bis zur Einhaltung der Geschäftslogik.
>>
>><div style="page-break-before: always;"></div>
>>
>>#### Prepared Statements
>>Übergabe von Parametern in Form von Platzhaltern in Modernen Programmiersprachen und API's.
>>- Verwendung Parametern statt Variablen in der Anweisung
>>- werden erst zur Laufzeit befüllt
>>
>>1. Definition der Struktur der SQL-Anweisung mit Platzhaltern für alle Eingabewerte:
>>```sql
>>$sql = $db->prepare("INSERT INTO Kunden (vorname, nachname) VALUES (?, ?)");
>>```
>>- Erstellung einer vorbereiteten Anweisung mit Prepare (Die Fragezeichen stellen Platzhalter für Werte dar.
>>
>>1. Spezifizierung der Inhalte der Platzhalter:
>>```sql
>>$sql->bind_param("ss", $vorname, $nachname);
>>```
>>- Ersetzung Platzhalter durch echte Werte mit Bind-Parameter-Funktion. Erster Parameter gibt an, welchen Typ folgende Werte haben (zwei Strings).
>>- Eingabedaten nicht als Teil der Befehlsstruktur betrachtet.
>> 
### Cross Site Scripting (XSS)
>[!abstract]
>- Einschleusen schadhafter Skripts in Webanwendungen.
>
**Charakteristik:**
>- Ausführung Skripte auf Nutzerseite und Ausnutzung von Sicherheitslücken, um fremden Code ungeprüft zu übernehmen
>- Richten sich gegen Clienten der Webanwendung als Grundlage für weitere Angriffe
>- Dienen oft Übernahme digitaler Identitäten (Cookies, Tokens), um an sensible Informationen zu gelangen.
>- Je nach Rechten der Identität kann gesamte Webanwendung manipuliert werden
>
>**Angriffsvektor:**
>- Wie bei SQL Injections nicht oder unzureichend gefilterte Benutzereingaben
>
>**Ziel:**
>- Angreifer auf Clientseite wird in Lage versetzt, schadhaften Code in Webseite einzuschleusen, der durch anderen Client wieder ausgeführt wird
>	- Angriffswege nicht auf klassische Webseiten und Eingabefelder beschränkt
>	- Schädliche Links auch in Onlineforen oder Apps
>	- Schadcode ebenso in Emails einbettbar
>
>**Methodik:**
>- Nutzung von Sicherheitslücken in Skriptsprachen (HTML, JavaScript, AJAX, ...)
>- in Webanwendungen eingeschleuster Schadcode wird von anderen Clients genauso behandelt wie der ursprüngliche Quelltext, da der gesamte Code aus einer sicheren Quelle (vom Web Server) stammt
>- Schadcode muss zur Logik der Benutzerschnittstelle passen und bis zur Ausführung durch den Browser des Clients unbemerkt bleiben
>- Systematisch alle Eingabemöglichkeiten einer Webseite ausprobieren, bis Schwachstelle zum einschleusen eines Skipts gefunden ist 
>
**Formen und Auftreten:**
>- **Persistentes XSS:** serverseitige Cross-Site-Scripting-Variante Schadcode permanent auf Server (gewöhnlich in Datenbank) der Webanwendung
>- **Reflektiertes XSS:** Schadcode nicht auf Server der Webanwendung, sondern Request mit Schadcode nach Linkaufruf an Webserver gesendet und ausgeführt
>- **DOM-basiertes XSS / lokales XSS**: clientbasierter nicht-persistenter Angriff, Manipulation URL-Parameters und Einschleusung direkt in Browser des Clients ohne Nutzung Webserver. Eingeschleuste Code ändert in schadhafter Weise Struktur des DOM.
>
>>[!success] **Schutzmaßnamen**
>>Schwierigkeit Implementierung geeigneter Schutzmaßnahmen liegt nicht in den Schutzmaßnahmen selbst. Bei Entwicklung von Webanwendungen gilt es alle Stellen zu identifizieren, an denen sich Schwachstellen befinden die ausgenutzt werden können Die meistverwendeten Schutzmaßnahmen sind:
>>- Eingabefilterung: Benutzereingaben filtern und unerwünschte Eingabedaten säubern
>>- Ausgabekodierung: Benutzerinhalte, die in HTTP-Antworten ausgeliefert werden, so kodieren, dass sie beim Empfänger nicht ausgeführt werden
>>- Escaping: Potenziellen Schadcode maskieren, sodass er nicht ausgeführt wird
>>- Security Header: Überprüfung der übertragenen Daten durch den Browser
>>- Content Security Policy: Verwendung von Skripten beschränken
>
>#### Escaping
>**HTML Entity Escaping:** Nutzung, um alle Eingabezeichen, für die es möglich ist, in passenden HTML-Entitäten umzuwandeln. PHP htmlentities() verwendbar. Weitere populäre Funktionen sind htmlspecialchars(), addslashes() und urlencode(). **URL Escaping:** Per URL übergebene kodierte Parameter werden maskiert. Ratsam, wenn innerhalb Anwendung URLs aufgerufen werden können (z.B. in href-Tags) 
>**Attribut Value Escaping:** Verhindert Einschleusung von Schadcode unter Verwendung von HTML-Attributen wie src oder href 
>**JavaScript Escaping:** Maskiert Script-Tags um Ausnutzung von Event Handlern, wie onclick, ontouch, onchange, onsubmit usw. zu verhindern
>
>#### Content Security Policy
>Content Security Policy (CSP): Strikte Trennung von Inhaltsdaten (HTML) und externen Daten die JavaScript Code enthalten. Veränderung verschiedener Quellcodebereiche für eine korrekte Umsetzung der CSP. Auslagerung von `<script>`-Blöcke in externe DateienErsetzung Event-Handler-Attribute (onclick, onchange, onselect, …) durch Event-Listener Umwandlung Funktionsaufrufe mit Strings (setTimeout, setInterval) in Inline-Funktionen:
>

---

<div style="page-break-before: always; min-height:1px;">&nbsp;</div>

---

>[!abstract]
>### Cross Site Request Forgery (CSRF)
>- Angreifer nutzt harmlos aussehende website um Browser des Benutzers anfragen an verwundbare Webanwendungen senden zu lassen
>Vorraussetzungen:
>- verwundbare Webanwendung
>- bösartige Website für Weiterleitung
>- Authe. Benutzer in verwundbaren Webanwendungen
>- Angreifer tritt nicht mit eigener Indentität in Erscheinung
>
>**Ablauf:**
>- Während aktiver Sitzung zwischen Benutzer und verwundbarer Webanwendung
>- Aufruf zusätzlicher Webseite des Angreifers mit Schadcode
>- Schadcode ruft Inhalte verwundbarer Webanwendung mit manipulierten bzw. vorgefertigten Übergabewerten auf
>
>Zwei mögliche Angriffsarten einer CSRF Attacke abhängig von der Webanwendung:
>
>- Webanwendung nutzt hauptsächlich GET-Anfragen
>- Webanwendung nutzt zur Übermittlung POST-Anfragen
>
>#### CSRF mittels GET-Anfragen
>![[Pasted image 20260129134108.png | 550]]
>
>#### CSRF mittels POST-Anfragen 
>![[Pasted image 20260129134205.png | 550]]
>
>>[!success] Schutzmaßnahmen
>>CSRF Gegenmaßnahmen sind bereits sehr stark ausgebaut:
>>- Client Blockierung: Von Seiten der Client Browser / Email Anwendung Nachladen von Bildern oder bestätigen von Seitenfremden Formularinhalten blockiert 
>>- Multi-Faktor-Authentifizierung: Vor allem im Bereich von Online Banking durch Verwendung eins zweiten Faktor für die Bestätigung außerhalb der Browser
>>- Referrer Header: Optionales HTTP-Headerfeld mit Daten über Absender der Anfrage (Ursprungs URL), leicht durch Browser änderbar (AdBlocker)
>>- CSRF Token: Verifizierung einer Sitzung von weiteren Elementen abhängig machen als nur Session Cookie, durch Nutzung eines weiteren CSRF Token für jede Sitzung (Double-Submit-Cookie-Verfahren), bzw. nach jeder Abfrage am Server mittels temporären Abfrage Token
>
>#### Clickjacking (UI Redress Attack)
>Umgehung von Schutzmaßnahmen, die auf Verwendung eines CSFR-Tokens basieren.
>
>Clickjacking bedeutet:
>
>- Angreifer legt zusätzliche harmlos wirkende Schicht über Ausgabe der Webanwendung die Opfer veranlasst bestimmte Eingaben durchzuführen
>- Tatsächlich werden Eingaben in Webanwendung getätigt 
>
>**Voraussetzung**: Einbettung ursprünglicher Webanwendung in Angreifer Anwendung. Per JavaScript werden Mauseingaben des Opfers eingefangen und an Webanwendung übertragen#
>
>**Schutzmaßnahme**: Content Security Policy und verhindern von X-Frame Options

