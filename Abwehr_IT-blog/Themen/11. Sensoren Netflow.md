...
>
>Sensoren müssen zwischen Vollständigkeit und Nichtredundanz abwägen.
>Der perfekte Sensor wäre vollständig und nichtredundant. Dies ist in der Praxis jedoch nicht erreichbar.
>
>- drei Gesichtspunkte:
...
>#### Vantage
>
![[Pasted image 20250108142222.png | 600]]
 >
 >jeder Sensor sieht etwas anderes -> siehe Graph 
 >An einer Kante sehen Sensoren nur den Verkehr auf dem Weg. 
 >An einem Knoten sehen Sensoren Verkehr aus mehreren Wegen. 
 >An einem Endpunkt -> nur für diesen bestimmten verkehr
>
>>[!example]
...
>>>
>>- A-E-G-H(ohne E)-> beste alternative

>[!abstract]
>#### Domains 
>Netzwerksensoren können Daten in drei Domains (Arbeitsgebieten) sammeln:
>- **Network:** Solche Sensoren sammeln Daten über den Netzwerkverkehr. Beispiele wären VPNs, die meisten IDS, NetFlow Collectoren, wie YAF, und TCP Collectoren wie Snort und Raw Data Collectoren wie tcpdump oder Wireshark
>- **Host:** Solche Sensoren sitzen auf einem Host und beobachten die Aktivitäten dort, wie Logins, Dateizugriffe, .. Beispiele sind: IPS Tools wie Tripwire, HIPS, Logles,
>- **Service:** Sensoren werden von einem einzelnem Service Prozess generiert. Sie sind nur auf einen einzelnen Service fokussiert. 
>
...
>
![[Pasted image 20250201221533.png | 550]]
>
>Unterschiede zwischen drei Sensortypen:
...
>	- Es muss bekannt sein dass und wo es existiert
...
>>[!example] **Beispiel: Nicht Relatable**
>>```bash
...
>>```
>
>>[!example] **Beispiel: Besser Relatable**
>>```bash
...
><div style="page-break-before: always;"></div>
>
>>[!example] **Beispiel: Nicht sehr Complete:**
>>```bash
...
>>```
>
>>[!example] **Beispiel: Besser und Complete**
>>```bash
>>Mar 29 11:22:45.521 myhost (192.168.2.2) myspamapp[213]:
...
>
>#### Auswertung
>Bei der Auswertung verschiedener Log-Dateien müssen diese sinnvoll zusammengefasst werden:
...
>>>
>>- Alternative zur Einschränkung des von tcpdump mitgeschnittenen Traffics sind die Berkeley Packet Filters (BPF)
>>- tcpdump zusammen mit pcap schneidet nur IP over Ethernet mit, für andere grundlegende Protokolle sind andere Tools von Nöten. (Fibre Channel, CAN Bus, ATM Asynchronous Transfer Mode in ISDN) 
>
...
>[!abstract]
>### <mark style="background: #FF5582A6;">Silk</mark> (System for Internet Level Knowlege)
>
...
>- Der Befehl um auf Daten zuzugreifen ist: `rwcut {Datei}` mit `--field` kann die Ausgabe beschränkt werden (rwcut) output:
>
![[Pasted image 20250201230041.png]]
>
>>[!example] Beispiele rwcut:
>>
>>```bash
...
>>
>>```bash
>>rwfilter --dport=80 {Datei} --pass=stout | rwcut --field=1-9 #zeigt alle Verbindungen von Port 80 
>>
...
>>```
>
![[Pasted image 20260131164107.png]]
>
...
>- **rwcount** fasst SilK-Daten zu Zeitreihen zusammen, indem er Records in bins gleicher Zeitdauer einsortiert.
>
>#### rwcount
